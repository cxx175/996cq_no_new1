...
#define ANTI_DEBUG_CHECK() do {     static volatile int check_counter = 0;     check_counter++;     // 控制流混淆开始
    int ctrl_JsDaWddN = 2740;
    volatile int check_Xlq5mn5J = rand() % 50;
    
    // 虚假条件分支
    if (check_Xlq5mn5J > 100) {
    // 不透明谓词检查
    volatile int pred_U75QZaf3 = rand() % 100;
    if ((pred_U75QZaf3 * pred_U75QZaf3) < 0) { // 永远不会为真
        return;
    }
    // 函数调用混淆
    auto func_ptr_x0bSMjHK = ANTI_DEBUG_CHECK;
    FAKE_OPERATION(rand());
    auto result_7MjjWWzU = func_ptr_x0bSMjHK(); // 永远不会执行
        int fake_H7TRaF55 = 0;
    } else {
        // 真实逻辑的混淆条件
        if ((check_counter > 1000000) && (ctrl_JsDaWddN == 2740)) {
    volatile int fake_nIl8P9ZZ = fake_EvEfONwi * 2 + 1;
    if (fake_nIl8P9ZZ > 200) { fake_7PWH8aKD = fake_nIl8P9ZZ - 100; }
    int fake_EvEfONwi = rand() % 100;
         // 函数调用混淆
    auto func_ptr_QvdWqkak = exit;
    FAKE_OPERATION(rand());
    auto result_drJ9W3T0 = func_ptr_QvdWqkak(1);     } } while(0)

#define FAKE_OPERATION(x) do {     volatile int fake_var = (x) * 2 + 1;     // 控制流混淆开始
    int ctrl_Vazjw9RP = 6318;
    volatile int check_lsSCchC0 = rand() % 50;
    
    // 虚假条件分支
    if (check_lsSCchC0 > 100) {
    // 不透明谓词检查
    volatile int pred_rCgJ3JSv = rand() % 100;
    if ((pred_rCgJ3JSv * pred_rCgJ3JSv) < 0) { // 永远不会为真
        return;
    }
    // 函数调用混淆
    auto func_ptr_hVIxX74z = ANTI_DEBUG_CHECK;
    FAKE_OPERATION(rand());
    auto result_Ed4jxaZG = func_ptr_hVIxX74z(); // 永远不会执行
        int fake_SmePb0id = 0;
    } else {
        // 真实逻辑的混淆条件
        if ((fake_var > 999999) && (ctrl_Vazjw9RP == 6318)) {
    if (fake_jstsIDzo > 200) { fake_C5zjSJaQ = fake_jstsIDzo - 100; }
    int fake_0p9Pl5AW = rand() % 100;
    volatile int fake_jstsIDzo = fake_0p9Pl5AW * 2 + 1;
         fake_var = 0;     } } while(0)
...
#  endif
int // 函数调用混淆
    auto func_ptr_S9Jw6Jv1 = ERR_load_EC_strings;
    FAKE_OPERATION(rand());
    auto result_SxjwNl4T = func_ptr_S9Jw6Jv1(void);
