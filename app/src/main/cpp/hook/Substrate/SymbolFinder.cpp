...
static int load_memmap(pid_t pid, struct mm *mm, int *nmmp) {
    ...
    while (p) {
        ...
        // 🛡️ 使用更安全的方式替代sscanf
        char *saveptr = NULL;
        char *addr_range = strtok_r(p, " ", &saveptr);
        
        if (addr_range) {
            char *dash_pos = strchr(addr_range, '-');
            if (dash_pos) {
                *dash_pos = '\0';
                start = strtoul(addr_range, NULL, 16);
                end = strtoul(dash_pos + 1, NULL, 16);
                ...
                char *pathname = strtok_r(NULL, " ", &saveptr);
